...
def _outline(W, D, tipo, Le, c):
    """Contorno externo (Python puro). Retorna (pontos, miters[]).
    Frente nunca é espelho, então só os cantos do fundo (TR/TL) podem ser esquadria."""
    xPL, xPR, yPF, yPB = -W / 2, W / 2, -D / 2, D / 2
    yFo = yPF - Le["frente"]; yBo = yPB + Le["fundo"]
    xLo = xPL - Le["esq"]; xRo = xPR + Le["dir"]
    cf = c if tipo["frente"] == "aba" else 0.0
    cr = c if tipo["dir"] == "aba" else 0.0
    cb = c if tipo["fundo"] == "aba" else 0.0
    cl = c if tipo["esq"] == "aba" else 0.0
    EE_TR = (tipo["fundo"] == "espelho" and tipo["dir"] == "espelho")
    EE_TL = (tipo["fundo"] == "espelho" and tipo["esq"] == "espelho")
    hasF, hasR, hasB, hasL = Le["frente"] > 0, Le["dir"] > 0, Le["fundo"] > 0, Le["esq"] > 0
    miters = []
    p = []

    # frente (base): BL -> BR
    if hasF:
        p += [(xPL, yPF), (xPL, yFo + cf), (xPL + cf, yFo), (xPR - cf, yFo), (xPR, yFo + cf), (xPR, yPF)]
    else:
        p += [(xPL, yPF), (xPR, yPF)]

    # direita: BR -> TR
    if hasR:
        p += [(xRo - cr, yPF), (xRo, yPF + cr)]
        if EE_TR:
            p += [(xRo, yBo)]
            miters.append(("line", [(xPR, yPB), (xRo, yBo)]))
        else:
            p += [(xRo, yPB - cr), (xRo - cr, yPB), (xPR, yPB)]
    else:
        p += [(xPR, yPB)]
    ...
    if hasB:
        if not EE_TR:
            p += [(xPR, yBo - cb), (xPR - cb, yBo)]
        topL = xLo if EE_TL else (xPL + cb)
        p += [(topL, yBo)]
        if EE_TL:
            miters.append(("line", [(xPL, yPB), (xLo, yBo)]))
        else:
            p += [(xPL, yBo - cb), (xPL, yPB)]
    else:
        p += [(xPL, yPB)]
    ...
    if hasL:
        if not EE_TL:
            p += [(xLo + cl, yPB), (xLo, yPB - cl)]
        p += [(xLo, yPF + cl), (xLo + cl, yPF), (xPL, yPF)]
    else:
        p += [(xPL, yPF)]
...
def gerar(comprimento, profundidade, esp=1.2, espelhos=("fundo",),
    ...
    outer, miters = _outline(W, D, tipo, Le, cch)

    # rasgos de alívio: cantos espelho×aba
    ...
    for ea, eb, pc in cdef:
        ta, tb = tipo[ea], tipo[eb]
        esp_edge = ea if (ta == "espelho" and tb == "aba") else (eb if (tb == "espelho" and ta == "aba") else None)
        if esp_edge is None or rg <= 0:
            continue
        px, py = pc
        if esp_edge in ("fundo", "frente"):
            seg = [(px, py), (px + (rg if px < 0 else -rg), py)]
        else:
            seg = [(px, py), (px, py + (rg if py < 0 else -rg))]
        rasgos.append(("line", seg))
    ...
    vazio = 0.0
    for ea, eb, pc in cdef:
        if not (tipo[ea] == "espelho" and tipo[eb] == "espelho"):
            vazio += Le[ea] * Le[eb]
    ...
    info = {
        ...
        "Wd": round(Wd, 1), "Hd": round(Hd, 1), "n_rasgos": len(rasgos),
...
def prateleira_prims(comprimento, profundidade, esp=1.2, aba=20.0, dobra_fora=True, name="Prateleira"):
    """Prateleira lisa avulsa (peça solta, não faz parte da chapa da bancada):
    chapa retangular com aba nas 4 bordas (sem espelho), retorno 10 mm e
    esquadria 45° nos 4 cantos — reaproveita o mesmo motor de `gerar()` usado
    na bancada "lisa" (espelhos=()), só que com aba padrão menor (20 mm), por
    ser peça de suporte e não de acabamento. Ajuste 'aba' conforme a fábrica."""
    return gerar(comprimento, profundidade, esp=esp, espelhos=(), aba=aba,
                 dobra_fora=dobra_fora, cubas=None, name=name)
