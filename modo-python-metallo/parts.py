...
def _sec_tubo(b, t, wall, raio=0.0):
    """Área da seção de tubo retangular b x t, parede wall, com raio de canto
    externo opcional (raio interno = max(raio - wall, 0))."""
    import math
    k = 4.0 - math.pi
    r = max(float(raio), 0.0)
    ri = max(r - wall, 0.0)
    outer = b * t - k * r * r
    inner = (b - 2 * wall) * (t - 2 * wall) - k * ri * ri
    return outer - inner
...
def mesa_pes_c(W, Dp, H, perfil=30.0, wall=1.5, top_th=25.0,
    ...
    if raio:
        name += f"_r{raio:g}"
    a2 = ang_diag / 2.0                      # esquadria junto à diagonal (topo)
    a1 = 90.0 - ang_diag / 2.0               # esquadria junto à diagonal (base)
    especs = [("Poste_fundo", Hq, 0, 45, 2),
              ("Aba_topo", flag, 45, a2, 2),
              ("Diagonal_frente", diag, a2, a1, 2),
              ("Pe_base", foot, 45, a1, 2)]
    if com_rail:
        especs.append(("Travessa_sup", L_rail, 0, 0, 1))
    pecas = _partes_tubo(b, wall, especs)
    return Result(
        parts=pecas,
        shape=asm.val().wrapped, solid=asm, name=name, kind="mesa_c",
        dims=(W, Dp, H), wall=wall, raio=raio, mass=massa, cut_list=cut_list,
...
# ---------------------------------------------------------------- 6b) mesas: pé de quadro, base X e cubo
def _partes_tubo(b, wall, especs):
    """Peças de tubo INDIVIDUAIS (ocas, com as esquadrias cortadas) para o
    nesting na laser de tubo (iTubeCAM/SinoCAM). especs = [(nome, L, ce, cd,
    qtd), ...] com ângulos em graus a partir do corte reto. Retorna
    dict nome -> sólido, no mesmo formato do mesa_metalon."""
    from .itubecam import tubo_peca
    out = {}
    for nome, L, ce, cd, qtd in especs:
        if L <= 0 or qtd <= 0:
            continue
        try:
            r = tubo_peca(b, b, wall, L, corte_esq=ce, corte_dir=cd,
                          plano="largura",
                          nome=f"{nome}_{b:g}x{b:g}_L{round(L)}_x{int(qtd)}")
        except ValueError as e:
            raise ValueError(f"Peça '{nome}' p/ laser de tubo: {e}")
        out[r.name] = r.solid
    return out


def _frame_rect(F, Hq, b, t, y0):
    """Quadro retangular fechado (moldura) no plano X x Z, espessura t ao longo
    de Y iniciando em y0. F = largura do quadro, Hq = altura, b = lado do tubo."""
    outer = [(0, 0), (F, 0), (F, Hq), (0, Hq)]
    inner = [(b, b), (F - b, b), (F - b, Hq - b), (b, Hq - b)]
    o = cq.Workplane("XY").polyline(outer).close().extrude(t)
    i = cq.Workplane("XY").polyline(inner).close().extrude(t)
    fr = o.cut(i).rotate((0, 0, 0), (1, 0, 0), 90).translate((0, y0 + t, 0))
    return fr, outer, inner


def mesa_pe_quadro(W, Dp, H, perfil=30.0, wall=1.5, top_th=25.0,
                   com_rail=True, inset_lateral=90.0, recuo_frente=0.0,
                   niveladores=False, nivelador_h=25.0, par=True, raio=0.0):
    """
    Mesa/escrivaninha com PÉ DE QUADRO (par de molduras retangulares fechadas).

    W, Dp, H    : largura x profundidade x altura montada (mm)
    perfil      : lado do tubo quadrado (mm)
    top_th      : espessura do tampo (mm)
    com_rail    : travessa superior ligando os dois quadros
    inset_lateral: recuo do quadro em relação à borda lateral do tampo
    recuo_frente: recuo do quadro em relação às bordas frente/fundo do tampo
    niveladores : pés niveladores reguláveis sob o quadro (altura nivelador_h)
    par         : False = gera UM quadro avulso (sem tampo e sem travessa)
    """
    b = float(perfil); t = float(perfil)
    niv_h = float(nivelador_h) if niveladores else 0.0
    Hq = H - top_th - niv_h
    F = Dp - 2.0 * float(recuo_frente)
    if F <= 2 * b + 10:
        raise ValueError("Recuo de frente grande demais para a profundidade.")
    if Hq <= 2 * b + 10:
        raise ValueError("Altura insuficiente para o perfil escolhido.")

    def _niv(frame, y0):
        for cx in (recuo_frente + b / 2, recuo_frente + F - b / 2):
            pe = (cq.Workplane("XY").transformed(offset=(cx, y0 + t / 2, -niv_h))
                  .circle(min(b, 30.0) / 2).extrude(niv_h))
            frame = frame.union(pe)
        return frame

    leg1, outer, inner = _frame_rect(F, Hq, b, t, inset_lateral if par else 0.0)
    if recuo_frente:
        leg1 = leg1.translate((recuo_frente, 0, 0))
    if niveladores:
        leg1 = leg1.translate((0, 0, niv_h))

    if not par:
        if niveladores:
            leg1 = _niv(leg1, 0.0)
        sec = _sec_tubo(b, t, wall, raio)
        massa = (2 * Hq + 2 * F) * sec * DENSIDADE
        cut_list = [
            (f"Montante vertical {b:g}x{t:g}", 2, round(Hq), "2 meia esquadria 45"),
            (f"Travessa do quadro {b:g}x{t:g}", 2, round(F), "2 meia esquadria 45"),
        ]
        if niveladores:
            cut_list.append(("Nivelador regulável (comprado)", 2, round(niv_h), "—"))
        name = f"Quadro_avulso_{F:g}x{H:g}_perfil{b:g}" + ("_niv" if niveladores else "")
        if raio:
            name += f"_r{raio:g}"
        pecas = _partes_tubo(b, wall, [("Montante", Hq, 45, 45, 2),
                                       ("Travessa_quadro", F, 45, 45, 2)])
        return Result(
            parts=pecas,
            shape=leg1.val().wrapped, solid=leg1, name=name, kind="mesa_quadro",
            dims=(F, t, H), wall=wall, raio=raio, mass=massa, cut_list=cut_list,
            extra=dict(F=F, Hq=Hq, perfil=b, top_th=0.0, com_rail=False,
                       inset=0.0, recuo=float(recuo_frente), avulso=True,
                       niveladores=niveladores, niv_h=niv_h,
                       outer=outer, inner=inner, L_rail=0.0),
        )

    leg2, _, _ = _frame_rect(F, Hq, b, t, W - inset_lateral - t)
    if recuo_frente:
        leg2 = leg2.translate((recuo_frente, 0, 0))
    if niveladores:
        leg2 = leg2.translate((0, 0, niv_h))
        leg1 = _niv(leg1, inset_lateral)
        leg2 = _niv(leg2, W - inset_lateral - t)

    top = cq.Workplane("XY").transformed(offset=(0, 0, Hq + niv_h)).box(Dp, W, top_th, centered=False)
    asm = leg1.union(leg2)
    L_rail = 0.0
    if com_rail:
        L_rail = W - 2 * inset_lateral - 2 * t
        rail = cq.Workplane("XY").transformed(
            offset=(recuo_frente + F / 2 - b / 2, inset_lateral + t, Hq + niv_h - b)).box(
            b, L_rail, b, centered=False)
        asm = asm.union(rail)
    asm = asm.union(top)

    sec = _sec_tubo(b, t, wall, raio)
    massa = (2 * (2 * Hq + 2 * F) + L_rail) * sec * DENSIDADE

    cut_list = [
        (f"Montante vertical {b:g}x{t:g}", 4, round(Hq), "2 meia esquadria 45"),
        (f"Travessa do quadro {b:g}x{t:g}", 4, round(F), "2 meia esquadria 45"),
    ]
    if com_rail:
        cut_list.append((f"Travessa superior {b:g}x{t:g}", 1, round(L_rail), "2 retas 90"))
    if niveladores:
        cut_list.append(("Nivelador regulável (comprado)", 4, round(niv_h), "—"))
    cut_list.append((f"Tampo {Dp:g}x{W:g} (esp. {top_th:g})", 1, round(W), "—"))

    name = f"Mesa_peQuadro_{W:g}x{Dp:g}x{H:g}_perfil{b:g}" + ("_niv" if niveladores else "")
    if raio:
        name += f"_r{raio:g}"
    especs = [("Montante", Hq, 45, 45, 4), ("Travessa_quadro", F, 45, 45, 4)]
    if com_rail:
        especs.append(("Travessa_sup", L_rail, 0, 0, 1))
    pecas = _partes_tubo(b, wall, especs)
    return Result(
        parts=pecas,
        shape=asm.val().wrapped, solid=asm, name=name, kind="mesa_quadro",
        dims=(W, Dp, H), wall=wall, raio=raio, mass=massa, cut_list=cut_list,
        extra=dict(F=F, Hq=Hq, perfil=b, top_th=top_th, com_rail=com_rail,
                   inset=inset_lateral, recuo=float(recuo_frente), avulso=False,
                   niveladores=niveladores, niv_h=niv_h,
                   outer=outer, inner=inner, L_rail=L_rail),
    )


def mesa_base_x(Wf, H, perfil=40.0, wall=1.5, tampo_D=0.0, raio=0.0):
    """
    BASE EM X (cruzeta): dois quadros retangulares iguais cruzados a 90 graus
    no centro (encaixe meia-madeira nas travessas). Indicada para tampos
    redondos ou quadrados apoiados por cima.

    Wf      : largura de cada quadro = envergadura da base (mm)
    H       : altura da base (mm) — o tampo assenta por cima
    tampo_D : diâmetro (ou lado) do tampo, apenas informativo na lista de corte
    """
    b = float(perfil); t = float(perfil)
    if Wf <= 2 * b + 10:
        raise ValueError("Envergadura pequena demais para o perfil.")
    y0 = Wf / 2.0 - t / 2.0
    frA, outer, inner = _frame_rect(Wf, H, b, t, y0)
    frB = frA.rotate((Wf / 2, Wf / 2, 0), (Wf / 2, Wf / 2, 1), 90)
    asm = frA.union(frB)

    sec = _sec_tubo(b, t, wall, raio)
    massa = 2 * (2 * H + 2 * Wf) * sec * DENSIDADE

    cut_list = [
        (f"Montante vertical {b:g}x{t:g}", 4, round(H), "2 meia esquadria 45"),
        (f"Travessa do quadro {b:g}x{t:g}", 4, round(Wf),
         "2 meia esq. 45 + meia-madeira central"),
    ]
    if tampo_D:
        cut_list.append((f"Tampo Ø/lado {tampo_D:g} (à parte)", 1, round(tampo_D), "—"))

    name = f"Base_X_{Wf:g}x{Wf:g}x{H:g}_perfil{b:g}"
    if raio:
        name += f"_r{raio:g}"
    pecas = _partes_tubo(b, wall, [("Montante", H, 45, 45, 4),
                                   ("Travessa_meia_madeira", Wf, 45, 45, 4)])
    return Result(
        parts=pecas,
        shape=asm.val().wrapped, solid=asm, name=name, kind="mesa_x",
        dims=(Wf, Wf, H), wall=wall, raio=raio, mass=massa, cut_list=cut_list,
        extra=dict(Wf=Wf, perfil=b, tampo_D=float(tampo_D),
                   outer=outer, inner=inner),
    )


def mesa_cubo(A, H, perfil=30.0, wall=1.5, top_th=20.0, moldura=True, raio=0.0):
    """
    PÉ CUBO: dois quadros cruzados a 90 graus posicionados nas DIAGONAIS do
    tampo quadrado, com moldura superior opcional contornando o tampo.

    A       : lado do tampo quadrado (mm)
    H       : altura montada (mm)
    top_th  : espessura do tampo (mm)
    moldura : True adiciona o quadro-borda superior (lado A) sob o tampo
    """
    import math
    b = float(perfil); t = float(perfil)
    Hq = H - top_th
    diag = A * math.sqrt(2.0)
    Hf = Hq - (b if moldura else 0.0)     # altura dos quadros cruzados
    if Hf <= 2 * b + 10:
        raise ValueError("Altura insuficiente para o perfil/moldura.")

    y0 = diag / 2.0 - t / 2.0
    frA, outer, inner = _frame_rect(diag, Hf, b, t, y0)
    frB = frA.rotate((diag / 2, diag / 2, 0), (diag / 2, diag / 2, 1), 90)
    cruz = frA.union(frB).rotate((diag / 2, diag / 2, 0), (diag / 2, diag / 2, 1), 45)
    off = A / 2.0 - diag / 2.0
    cruz = cruz.translate((off, off, 0))
    asm = cruz

    sec = _sec_tubo(b, t, wall, raio)
    massa = 2 * (2 * Hf + 2 * diag) * sec * DENSIDADE

    if moldura:
        ring = _box(0, A, 0, A, Hf, Hf + b).cut(_box(b, A - b, b, A - b, Hf - 1, Hf + b + 1))
        asm = asm.union(ring)
        massa += 4 * (A - b) * sec * DENSIDADE

    top = cq.Workplane("XY").transformed(offset=(0, 0, Hq)).box(A, A, top_th, centered=False)
    asm = asm.union(top)

    cut_list = [
        (f"Montante vertical {b:g}x{t:g}", 4, round(Hf), "2 meia esquadria 45"),
        (f"Travessa diagonal {b:g}x{t:g}", 4, round(diag),
         "2 meia esq. 45 + meia-madeira central"),
    ]
    if moldura:
        cut_list.append((f"Moldura superior {b:g}x{t:g}", 4, round(A), "2 meia esquadria 45"))
    cut_list.append((f"Tampo {A:g}x{A:g} (esp. {top_th:g})", 1, round(A), "—"))

    name = f"Mesa_cubo_{A:g}x{A:g}x{H:g}_perfil{b:g}" + ("_moldura" if moldura else "")
    if raio:
        name += f"_r{raio:g}"
    especs = [("Montante", Hf, 45, 45, 4),
              ("Travessa_diagonal", diag, 45, 45, 4)]
    if moldura:
        especs.append(("Moldura_sup", A, 45, 45, 4))
    pecas = _partes_tubo(b, wall, especs)
    return Result(
        parts=pecas,
        shape=asm.val().wrapped, solid=asm, name=name, kind="mesa_cubo",
        dims=(A, A, H), wall=wall, raio=raio, mass=massa, cut_list=cut_list,
        extra=dict(A=A, Hq=Hq, Hf=Hf, diag=diag, perfil=b, top_th=top_th,
                   moldura=moldura, outer=outer, inner=inner),
    )


def _offset_inner_quad(pts, b):
    """Polígono interno de um quadrilátero convexo, deslocando cada aresta
    'b' para dentro (interseção de retas — puro Python)."""
    import math
    n = len(pts)
    # área assinada > 0 => anti-horário; normal interna = rot90 esquerda
    ...
    for i in range(n):
        ...
        dx, dy = x2 - x1, y2 - y1
        L = math.hypot(dx, dy)
        nx, ny = -dy / L * sgn, dx / L * sgn      # normal para dentro
        linhas.append((x1 + nx * b, y1 + ny * b, dx, dy))
    inner = []
    for i in range(n):
        x1, y1, dx1, dy1 = linhas[i - 1]
        x2, y2, dx2, dy2 = linhas[i]
        den = dx1 * dy2 - dy1 * dx2
        s = ((x2 - x1) * dy2 - (y2 - y1) * dx2) / den
        inner.append((x1 + s * dx1, y1 + s * dy1))
    return inner


def mesa_pe_trapezio(W, Dp, H, base=None, perfil=30.0, wall=1.5, top_th=25.0,
                     com_rail=False, inset_lateral=90.0, raio=0.0):
    """
    Mesa com PÉ TRAPÉZIO (par): quadro trapezoidal simétrico — barra superior
    na largura total do tampo (Dp) e base menor, montantes inclinados.

    W, Dp, H : largura x profundidade x altura montada (mm)
    base     : largura da base no chão (default 0.62*Dp)
    """
    import math
    b = float(perfil); t = float(perfil)
    Hq = H - top_th
    Db = float(base) if base else 0.62 * Dp
    Db = min(Db, Dp - 1.0)
    off = (Dp - Db) / 2.0
    if Db <= 2 * b + 10 or Hq <= 2 * b + 10:
        raise ValueError("Base ou altura insuficiente para o perfil escolhido.")

    diag = math.hypot(off, Hq)
    ang = math.degrees(math.atan2(Hq, off)) if off > 0 else 90.0

    outer = [(off, 0), (off + Db, 0), (Dp, Hq), (0, Hq)]
    inner = _offset_inner_quad(outer, b)
    o = cq.Workplane("XY").polyline(outer).close().extrude(t)
    i = cq.Workplane("XY").polyline(inner).close().extrude(t)
    leg1 = o.cut(i).rotate((0, 0, 0), (1, 0, 0), 90).translate((0, inset_lateral + t, 0))
    leg2 = o.cut(i).rotate((0, 0, 0), (1, 0, 0), 90).translate((0, W - inset_lateral, 0))
    ...
    if com_rail:
        L_rail = W - 2 * inset_lateral - 2 * t
        rail = cq.Workplane("XY").transformed(
            offset=(Dp / 2 - b / 2, inset_lateral + t, Hq - b)).box(b, L_rail, b, centered=False)
        asm = asm.union(rail)
    asm = asm.union(top)

    sec = _sec_tubo(b, t, wall, raio)
    massa = (2 * (Dp + Db + 2 * diag) + L_rail) * sec * DENSIDADE

    cut_list = [
        (f"Barra superior {b:g}x{t:g}", 2, round(Dp), f"2 esquadrias (~{ang:.0f} graus)"),
        (f"Barra da base {b:g}x{t:g}", 2, round(Db), f"2 esquadrias (~{ang:.0f} graus)"),
        (f"Montante inclinado {b:g}x{t:g}", 4, round(diag), f"2 esquadrias (~{ang:.0f} graus)"),
    ]
    if com_rail:
        cut_list.append((f"Travessa superior {b:g}x{t:g}", 1, round(L_rail), "2 retas 90"))
    cut_list.append((f"Tampo {Dp:g}x{W:g} (esp. {top_th:g})", 1, round(W), "—"))

    name = f"Mesa_peTrapezio_{W:g}x{Dp:g}x{H:g}_perfil{b:g}"
    if raio:
        name += f"_r{raio:g}"
    a_top = ang / 2.0                        # esquadria dos cantos superiores
    a_base = 90.0 - ang / 2.0                # esquadria dos cantos da base
    especs = [("Barra_superior", Dp, a_top, a_top, 2),
              ("Barra_base", Db, a_base, a_base, 2),
              ("Montante_inclinado", diag, a_top, a_base, 4)]
    if com_rail:
        especs.append(("Travessa_sup", L_rail, 0, 0, 1))
    pecas = _partes_tubo(b, wall, especs)
    return Result(
        parts=pecas,
        shape=asm.val().wrapped, solid=asm, name=name, kind="mesa_trapezio",
        dims=(W, Dp, H), wall=wall, raio=raio, mass=massa, cut_list=cut_list,
        extra=dict(Db=Db, off=off, Hq=Hq, diag=diag, ang=ang, perfil=b,
                   top_th=top_th, com_rail=com_rail, inset=inset_lateral,
                   outer=outer, inner=inner, L_rail=L_rail),
    )


def mesa_estrutura_quadro(L, Dp, H, perfil=30.0, wall=1.5, travessa_central=True, raio=0.0):
    """
    ESTRUTURA COMPLETA: moldura superior perimetral + pés em quadro fechado
    (U soldado à moldura) nas duas pontas + travessa central longitudinal
    opcional. Base para tampo de vidro, madeira ou pedra (apoiado por cima).

    L, Dp, H : comprimento x profundidade x altura da estrutura (mm)
    """
    b = float(perfil); t = float(perfil)
    if H <= 2 * b + 10 or Dp <= 2 * b + 10 or L <= 2 * b + 10:
        raise ValueError("Dimensões insuficientes para o perfil escolhido.")

    # moldura superior (anel L x Dp), altura b, z em [H-b, H]
    ring = _box(0, L, 0, Dp, H - b, H).cut(_box(b, L - b, b, Dp - b, H - b - 1, H + 1))

    # pés: em cada ponta, 2 montantes verticais + travessa inferior (fecha o U com a moldura)
    P = [ring]
    for cx in (b / 2, L - b / 2):
        for cy in (b / 2, Dp - b / 2):
            P.append(_box(cx - b / 2, cx + b / 2, cy - b / 2, cy + b / 2, 0, H - b))
    for cx in (b / 2, L - b / 2):
        P.append(_box(cx - b / 2, cx + b / 2, b, Dp - b, 0, b))     # travessa inferior da ponta
    ...
    if travessa_central:
        Lc = L - 2 * b
        asm = asm.union(_box(b, L - b, Dp / 2 - b / 2, Dp / 2 + b / 2, H - b, H))

    sec = _sec_tubo(b, t, wall, raio)
    comp_total = 2 * L + 2 * Dp + 4 * (H - b) + 2 * (Dp - 2 * b) + Lc
    massa = comp_total * sec * DENSIDADE

    cut_list = [
        (f"Longarina da moldura {b:g}x{t:g}", 2, round(L), "2 meia esquadria 45"),
        (f"Travessa da moldura {b:g}x{t:g}", 2, round(Dp), "2 meia esquadria 45"),
        (f"Montante vertical {b:g}x{t:g}", 4, round(H - b), "2 retas 90"),
        (f"Travessa inferior da ponta {b:g}x{t:g}", 2, round(Dp - 2 * b), "2 retas 90"),
    ]
    if travessa_central:
        cut_list.append((f"Travessa central {b:g}x{t:g}", 1, round(Lc), "2 retas 90"))

    name = f"Estrutura_quadro_{L:g}x{Dp:g}x{H:g}_perfil{b:g}"
    if raio:
        name += f"_r{raio:g}"
    especs = [("Longarina_moldura", L, 45, 45, 2),
              ("Travessa_moldura", Dp, 45, 45, 2),
              ("Montante", H - b, 0, 0, 4),
              ("Travessa_inferior", Dp - 2 * b, 0, 0, 2)]
    if travessa_central:
        especs.append(("Travessa_central", Lc, 0, 0, 1))
    pecas = _partes_tubo(b, wall, especs)
    return Result(
        parts=pecas,
        shape=asm.val().wrapped, solid=asm, name=name, kind="mesa_estrutura",
        dims=(L, Dp, H), wall=wall, raio=raio, mass=massa, cut_list=cut_list,
        extra=dict(perfil=b, travessa_central=travessa_central, Lc=Lc),
    )
